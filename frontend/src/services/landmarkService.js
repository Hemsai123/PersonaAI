 ...
 */
export const identifyFamousLandmarks = async (cityName) => {
  ...
  Return the result ONLY as a JSON array of objects with keys "name" and "description".`;
    try {
        const response = await generateChatResponse("You are a travel expert with world-class knowledge of landmarks. Return only a raw JSON array. Never include explanations.", [], prompt);
 ...
 */
export const geocodeLandmark = async (name, cityName) => {
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            const bestMatch = data.find(item => {
                ...
                return ((addr.city || '').toLowerCase().includes(cityLower) ||
 ...
 */
export const fetchLandmarkImage = async (name) => {
