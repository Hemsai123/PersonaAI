...
export class TextToSpeech {
    constructor() {
        this.utterance = null;
        this.isPaused = false;
        this.selectedVoice = VOICE_OPTIONS[0].value;
        this.voices = [];
    ...
    findMatchingVoice(voiceId) {
        ...
        let voice = this.voices.find(v => v.name.toLowerCase().includes(voiceOption.name.toLowerCase()) &&
            v.lang.startsWith(voiceOption.accent.split(' ')[0].toLowerCase()));
        // Fall back to accent match
        if (!voice) {
            voice = this.voices.find(v => v.lang.startsWith(voiceOption.accent.split(' ')[0].toLowerCase()));
        }
    ...
    speak(text) {
        try {
            ...
            throw new SpeechError('Failed to start text-to-speech playback', error);
        }
    }
    pause() {
